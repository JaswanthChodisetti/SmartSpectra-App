...
SAVE_PATH = ROOT / "models/checkpoints/model2_balanced_boost.pkl"
...
def augment_features(features, n_variants=5):
    """
    Create synthetic variations of spectral features by adding small random noise.
    This simulates instrument noise and slight sample variance.
    """
    variants = []
    for _ in range(n_variants):
        # Add very small Gaussian noise (std = 0.001 * std of features)
        noise = np.random.normal(0, 0.001 * np.std(features), size=features.shape)
        variants.append(features + noise)
    return np.array(variants)

def train_balanced_boost():
    print("--- BALANCED DIVERSIFICATION TRAINING (The Safety Path) ---")
    ...
    print("Applying Balanced Augmentation...")
    X_aug, y_aug = [], []
    ...
    for label in [0, 1]:
        ...
        for feat in class_features:
            variants = augment_features(feat, n_variants=5)
    ...
    # 6. Safe Weighting: 2.0x for Fresh
    ...
    model.fit(X_train_scaled, y_train, sample_weight=final_weights, eval_set=[(X_val_scaled, y_val)], verbose=False)
    ...
    print("\n" + "="*40)
    print("BALANCED BOOST TEST RESULTS")
    print("="*40)
    ...
    if acc > 0.80 and pest_recall > 0.80:
        ...
        print("\nWarning: Model did not meet the 80% accuracy/safety threshold. Not saving.")

if __name__ == "__main__":
    train_balanced_boost()
